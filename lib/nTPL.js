...
*/
this.nTPL = nTPL = (function(compile, $tab , gid ,
	...
	*/
		...
		*/
		function parse(str, callback) {			
			...
			if (hasCode = str.match($brackets)) {
				...
				var parsed = nTPLnative.parse(str, $.modificators, namespace);
				
				// parsed = {replacements: [], code: []}
				var compiled = parsed.code.join("");
				var $rep = parsed.replacements;
				...
				try {
					...
					i = compile("(function(nTPL,$scope,$args,$rep," + args.join(",") + "){$_=[];function $p(a,$_){$_[$_.length]=a};" + compiled + ";return $_.join('')})", "nTPL.js");
	...
	return $;
		 ...
		 {} , /{%|%}/g ,
		 /^([^\s]+)(?:\s|$)/ , /\t/g ,
		 /\s+/g , /%1/);/**@preserve nTPL v.0.0.2;Copyright 2010, Fedor Indutny;Released under MIT license;Parts of code derived from jQuery JavaScript Library;Copyright (c) 2009 John Resig **/