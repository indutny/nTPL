...
this.parse = nTPL = (function() {	
	...
	*/
	function $main(str , args, name, callback, watch, watchCallback) {
		...
		*/
		function parse(str, callback) {			
			...
			if (hasCode = parsed && (parsed.replVars.length > 1 || parsed.code.length > 0 )) {
				...
				try {
					...
					i = scopedEval("(function(" + replVars + "){\
						return (function($args," + args.join(",") + "){\
							$_=[];\
			...
			if (this === REF_CHECK)
				return {i: i, args: args, name: name};
			...
			*/
			...
			refreshTemplate[refresh_id] = replaceScope = function(o) {
				...
				if (name != i.name) {
					// Copy
					namecache[i.name] = namecache[name];
					
					// Delete old
					namecache[name] = undefined;
				}
	...
	return $;
})();/**@preserve nTPL v.0.3.0;Copyright 2010, Fedor Indutny;Released under MIT license;Parts of code derived from jQuery JavaScript Library;Copyright (c) 2009 John Resig **/