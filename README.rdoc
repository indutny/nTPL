...
* Also nTPL have many optimizations in template rendering process.
* nTPL uses buffers
* nTPL is extendable (nTPL.modificators)
* nTPL can reload template on file change (watch: true)

==Build instructions:
Only nTPL.native need to be builded.
After that all nTPL module will be stored in lib folder:
(nTPL.js, nTPL.block.js, nTPL.filter.js, nTPL.native.node)
